// Auto-generated search metadata
export const SEARCH_INDEX = [
  ...
  },
  {
    ...
    "keywords": "verify each unit of logic in isolation, fast and repeatably. [fact] marks a single test; [theory] with [inlinedata] runs it with multiple inputs mock dependencies with moq so tests don't touch real databases or external services arrange\u2013act\u2013assert: set up state, call the code, assert the outcome unit tests verify a single class or method in isolation \u2014 no database, no http, no file system. the most popular framework in .net is xunit , which discovers tests by convention: any public class with a [fact] (single case) or [theory] (multiple inputs via [inlinedata] ) method is a test. assertions use assert.equal , assert.true , assert.throws , etc. isolating dependencies is done with mocks \u2014 test doubles that fake external behaviour. the moq library is the standard choice: new mock&lt;imyservice&gt;() creates a mock, .setup(...).returns(...) configures it, and .verify(...) asserts it was called. the arrange-act-assert pattern keeps tests readable: set up, run, check."
  },
  {
    "id": "solid-principles",
    "tagline": "Five principles that keep object-oriented code maintainable as it grows.",
    "keywords": "five principles that keep object-oriented code maintainable as it grows. srp: one reason to change \u2014 split classes when they serve two distinct purposes ocp + dip: depend on interfaces so new behaviour is added by adding code, not editing it lsp: a subclass that breaks callers of the base class is a design smell, not just a bug solid is an acronym for five design principles: single responsibility (a class should have one reason to change), open/closed (open for extension, closed for modification), liskov substitution (subtypes must be substitutable for their base types), interface segregation (prefer small, focused interfaces), and dependency inversion (depend on abstractions, not concretions). they're not rules to follow mechanically, but heuristics that point toward the same underlying goal: loosely coupled, highly cohesive code that can be extended and tested without cascading changes. violating one principle typically makes another harder to satisfy too \u2014 spotting that tension is often the clearest signal a refactor is needed."
  },
  {
    "id": "clean-architecture",
    "tagline": "Organise code in layers where dependencies only point inward.",
    "keywords": "organise code in layers where dependencies only point inward. dependencies point inward \u2014 domain knows nothing about infrastructure or the api layer application layer defines interfaces; infrastructure implements them (dependency inversion) outer layers reference inner layers, never the reverse \u2014 enforced by project references clean architecture (robert c. martin) organises a codebase into concentric layers \u2014 domain at the centre (entities, value objects, domain logic), application around it (use cases, interfaces), infrastructure outside (database, email, http clients), and presentation at the edge (api controllers, razor pages). the dependency rule : nothing in an inner layer ever references an outer one. this means your domain and application logic \u2014 the code that actually matters \u2014 has zero dependency on asp.net core, entity framework, or any other framework. it can be tested with plain unit tests, and the infrastructure can be swapped without touching business logic. in .net this is typically implemented with separate projects: domain , application , infrastructure , and api ."
  },
  {
    "id": "memory-management",
    "tagline": "Understand the GC, the stack vs heap, and how to avoid common leaks.",
    "keywords": "understand the gc, the stack vs heap, and how to avoid common leaks. gc is generational \u2014 gen 0 is cheap; large or long-lived objects in gen 2 are expensive to collect idisposable/using is mandatory for unmanaged resources \u2014 the gc won't call dispose span<t> and arraypool<t> reduce allocations and gc pressure in performance-critical paths .net uses a generational garbage collector that automatically reclaims heap memory. objects are born in generation 0 ; those that survive a collection are promoted to gen 1, then gen 2. short-lived objects (most objects) are collected cheaply in gen 0. long-lived objects (caches, singletons) sit in gen 2 and are collected rarely \u2014 if they grow unbounded, that's a memory leak even under gc. value types (structs, primitives) live on the stack (or inline in an array) and require no gc at all \u2014 they're simply popped when the method returns. span&lt;t&gt; and arraypool&lt;t&gt; are key tools for reducing heap allocations in hot paths. for anything that wraps an unmanaged resource (file handles, sockets), implement idisposable and use a using statement \u2014 the gc does not call dispose for you."
  },
  {
    "id": "performance-tips",
    "tagline": "Measure first, then apply the right tool \u2014 most performance gains come from a handful of patterns.",
    "keywords": "measure first, then apply the right tool \u2014 most performance gains come from a handful of patterns. profile before optimising \u2014 benchmarkdotnet and dotnet-trace show where time actually goes reduce allocations in hot paths: span<t>, arraypool, stringbuilder, struct-based types async/await for i/o; parallel.for for cpu-bound work; never block an async call with .result performance in .net nearly always starts with measurement : benchmarkdotnet for micro-benchmarks, dotnet-trace or visual studio's profiler for application-level profiling. guessing without data wastes effort and risks making things worse. the highest-leverage patterns: avoid allocations in hot paths (use span&lt;t&gt; , arraypool&lt;t&gt; , stringbuilder ); prefer async/await for i/o so threads aren't blocked; use parallel.foreach or plinq only for cpu-bound work; avoid async void , unnecessary task.result , and thread.sleep ; and keep linq chains over large sets out of hot paths \u2014 they allocate enumerators and delegates. valuetask instead of task avoids allocations when a result is often available synchronously."
  },
  {
    "id": "async-await",
  ...
  },
  {
    ...
    "keywords": "limit concurrency to a fixed number of workers. semaphoreslim(n) allows up to n concurrent callers through wait/waitasync waitasync() doesn't block a thread, unlike lock \u2014 ideal for async throttling always release() in a finally block to avoid permanently starving other callers while lock allows only one thread in at a time, semaphoreslim allows a configurable number of concurrent callers \u2014 useful for throttling access to a limited resource, like capping how many http requests run in parallel. unlike lock , it has an async-friendly waitasync() that doesn't block a thread while waiting, making it the right choice for throttling concurrent async work rather than raw multithreading."
  },
  {
    "id": "channels",
  ...
  },
  {
    ...
    "keywords": "chain request-handling components, each deciding whether to pass control forward. each middleware calls await next(context) to pass to the next component app.use adds pass-through middleware; app.run adds a terminal one order is critical \u2014 auth, routing, and endpoint middleware must be in the right sequence in asp.net core, every http request passes through a middleware pipeline \u2014 an ordered chain of components, each of which can inspect the request, do work, call the next component, and then do more work on the way back out. you compose the pipeline in program.cs using app.use , app.run , and app.map . app.use adds a middleware that calls next() to continue the chain. app.run is a terminal middleware \u2014 it ends the pipeline there. order matters: a middleware registered before another wraps it completely, so authentication must come before authorization, and routing before endpoint execution."
  },
  {
    "id": "minimal-apis",
    "tagline": "Build HTTP endpoints with almost no ceremony \u2014 no controllers, no attributes.",
    "keywords": "build http endpoints with almost no ceremony \u2014 no controllers, no attributes. mapget/mappost/mapput/mapdelete register endpoints with zero boilerplate parameters are automatically bound from route, query string, or json body mapgroup adds a shared prefix and middleware to a set of related endpoints minimal apis , introduced in .net 6, let you define http endpoints directly in program.cs using mapget , mappost , mapput , and mapdelete \u2014 no controller classes or action-method attributes required. the route handler is just a lambda or a named method, and asp.net core automatically binds parameters from the route, query string, or request body. route groups ( app.mapgroup ) let you add a common prefix and shared middleware to a set of endpoints. for larger apps, handler methods can be organized into separate files using extension methods on webapplication \u2014 keeping the conciseness without forcing everything into one file."
  },
  {
    "id": "authentication",
    "tagline": "Authentication proves who you are; authorization decides what you can do.",
    "keywords": "authentication proves who you are; authorization decides what you can do. useauthentication() and useauthorization() must be added in that order [authorize] protects a controller or endpoint; [allowanonymous] opens it back up policies group authorization rules by name, applied via requireauthorization(\"policyname\") authentication in asp.net core establishes the user's identity \u2014 who is making the request. authorization uses that identity to decide whether the request is permitted. the two are always separate: you authenticate first, then authorize. both are added as middleware in the pipeline, and order matters \u2014 useauthentication must come before useauthorization . the [authorize] attribute (or requireauthorization() in minimal apis) protects endpoints. policies let you express complex rules \u2014 role checks, claim requirements, or custom logic \u2014 in one named unit rather than scattering conditions across your code."
  },
  {
    "id": "jwt",
    "tagline": "Stateless, self-contained tokens for authenticating API requests.",
    "keywords": "stateless, self-contained tokens for authenticating api requests. jwts are signed, not encrypted \u2014 never put secrets in the payload validate issuer, audience, and expiry \u2014 all three matter for real security short expiry (15 min) + refresh tokens is the standard production pattern a json web token (jwt) is a compact, url-safe string carrying claims \u2014 assertions about a user, like their id or roles \u2014 that are cryptographically signed so a server can verify them without hitting a database. it has three base64-encoded parts separated by dots: header (algorithm), payload (claims), and signature . in asp.net core, the addjwtbearer extension registers middleware that reads the authorization: bearer &lt;token&gt; header, validates the signature and expiry against your configured options, and populates httpcontext.user with the token's claims \u2014 making them available to authorization policies and your own code."
  },
  {
    "id": "background-services",
    "tagline": "Run long-lived work alongside your app without blocking requests.",
    "keywords": "run long-lived work alongside your app without blocking requests. backgroundservice.executeasync runs for the app's lifetime \u2014 loop and await inside it always respect the cancellationtoken so the service shuts down cleanly register with builder.services.addhostedservice<myservice>() a hosted service in asp.net core implements ihostedservice (or inherits backgroundservice ) and runs for the lifetime of the application \u2014 started when the host starts, stopped gracefully when it shuts down. common uses include polling external apis, processing a message queue, sending scheduled emails, or cleaning up old records. backgroundservice is the abstract base class that wraps ihostedservice in a simpler api: you override executeasync , loop inside it (using the provided cancellationtoken to detect shutdown), and the framework handles start/stop for you. register it with addhostedservice&lt;t&gt; ."
  },
  {
    "id": "caching",
    "tagline": "Store expensive results and return them instantly on repeat requests.",
    "keywords": "store expensive results and return them instantly on repeat requests. imemorycache is in-process and fast; idistributedcache (redis) is shared across servers getorcreate/getorcreateasync combines lookup + set in one safe call always set expiration \u2014 unbounded caches grow until the process runs out of memory .net offers two main in-process caching abstractions: imemorycache stores data in the process's memory (fast, single-server), and idistributedcache (backed by redis or sql server) shares a cache across multiple instances. both let you set absolute and sliding expiration policies. for the common pattern of \"get from cache, or compute and store if missing\", imemorycache.getorcreate combines the lookup and write into one atomic call. the newer hybridcache (available from .net 9) merges both tiers and protects against cache stampedes \u2014 multiple requests all computing the same value simultaneously because the cache just expired."
  },
  {
    "id": "logging",
    "tagline": "Structured, levelled, provider-agnostic logging built into the framework.",
    "keywords": "structured, levelled, provider-agnostic logging built into the framework. ilogger<t> is the standard abstraction \u2014 inject it, never construct a logger directly use message templates (\"user {id} logged in\"), not string interpolation, for structured logging log levels (trace, debug, info, warning, error, critical) are filterable per category in config asp.net core ships with a built-in ilogger&lt;t&gt; abstraction that works with multiple providers (console, debug, eventsource, third-party like serilog or nlog) without your code ever knowing which one is active. inject ilogger&lt;myclass&gt; into a constructor and call log.information , log.warning , log.error , etc. prefer structured logging over string interpolation: use message templates with named placeholders ( \"processed {count} items\" ). providers that support structured data (seq, application insights, elastic) store these as queryable fields rather than raw strings, making logs far easier to search and aggregate in production."
  },
  {
    "id": "configuration",
    "tagline": "Read settings from JSON, environment variables, and secrets \u2014 in priority order.",
    "keywords": "read settings from json, environment variables, and secrets \u2014 in priority order. providers layer in priority order \u2014 env vars override appsettings.json by default getsection(\"name\").get<t>() binds a config section to a strongly-typed poco use dotnet user-secrets in development; never commit secrets to source control asp.net core's configuration system layers multiple providers on top of each other: appsettings.json , appsettings.{environment}.json , environment variables, command-line arguments, and (in development) user secrets. later providers win, so an environment variable always overrides a json file value. access individual values with iconfiguration[\"key\"] , or bind an entire section to a strongly-typed class with getsection(\"mysection\").get&lt;myoptions&gt;() \u2014 which pairs naturally with the options pattern for validation and di-friendly access. never hardcode connection strings or secrets \u2014 use environment variables or a secrets manager instead."
  },
  {
    "id": "options-pattern",
    "tagline": "Bind configuration to strongly-typed classes and inject them anywhere.",
    "keywords": "bind configuration to strongly-typed classes and inject them anywhere. ioptions<t> is singleton; ioptionssnapshot<t> is per-request; ioptionsmonitor<t> is live-updating validatedataannotations() fails fast at startup if required config is missing keep option classes simple pocos \u2014 no di, no logic, just properties the options pattern wraps the raw iconfiguration api in a di-friendly, strongly-typed layer. you define a plain class (no attributes, no base class), register it with services.configure&lt;myoptions&gt;(config.getsection(\"mysection\")) , then inject ioptions&lt;myoptions&gt; where you need it. three interfaces give different behaviours: ioptions&lt;t&gt; is a singleton \u2014 values are read once; ioptionssnapshot&lt;t&gt; re-reads per request (scoped); ioptionsmonitor&lt;t&gt; re-reads in real-time and fires a callback when config changes. addoptions&lt;t&gt;().validatedataannotations() adds startup validation so a missing required field fails fast, before any request hits your code."
  }
];
